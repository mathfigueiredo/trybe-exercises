...
function addTech(element) {
  for (let i = 0; i < container.childElementCount; i++) {
    if (container.children[i].id === element.target.id) {
      container.children[i].classList.add('tech');
    } else {
      container.children[i].classList.remove('tech');
    }
  }
}
...
myWebpage.addEventListener('dblclick', redirectStack);
